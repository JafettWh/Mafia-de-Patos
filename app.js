...
// ==========================================
// AUDIO — AudioContext lazy (requerido por navegadores modernos)
// Se crea en el primer gesto del usuario, no al cargar la página
// ==========================================
const SoundEffects = {
    ctx: null,
    getCtx() {
    ...
    play(type) {
        try {
            ...
            if (type === 'click') {
                ...
                osc.start();
                osc.stop(this.getCtx().currentTime + 0.05);
                ...
                osc.start();
                osc.stop(this.getCtx().currentTime + 0.35);
                ...
                osc.start();
                osc.stop(this.getCtx().currentTime + 0.5);
            }
        } catch(e) {}
    }
};
...
function syncGamePhase(phase) {
    if (phase === 'ASSIGNMENT') {
        // Si el jugador ya tiene mafia asignada, mostrar pantalla de asignación
        if (myMafiaId) enterAssignmentPhase();
        // Solo el host ve el botón para lanzar la primera ronda
        if (isHost) {
            let btn = document.getElementById('btn-launch-dashboard');
...
// Lanza la primera ronda desde la pantalla de asignación
...
function enterAssignmentPhase() {
    ...
    // Evitar duplicar el listener si se llama varias veces
    let saveBtn = document.getElementById('btn-save-mafia-name');
...
function renderDashboard() {
    ...
    // Actualizar número de ronda en el header
    let roundLabel = document.getElementById('timer-label');
    ...
    // Poblar selector de mafias objetivo
    ...
    // Clonar botones para evitar listeners duplicados entre rondas
...
function renderTransition() {
    ...
    let logBox = document.getElementById('round-narrative-log');
    logBox.innerHTML = "";
    ...
    if (isHost) {
        document.getElementById('btn-admin-next').classList.remove('hidden');
    }
    document.getElementById('player-wait-next').style.display = isHost ? 'none' : 'block';
}

// ==========================================
// LÓGICA DE RESOLUCIÓN DE RONDA
// ==========================================
    ...
    db.ref('game_room').once('value', (snap) => {
        ...
        if (rVotes) {
            Object.values(rVotes).forEach(v => {
                if (!mafiaDecisions[v.mafiaSource].votesCount[v.action]) {
                    mafiaDecisions[v.mafiaSource].votesCount[v.action] = 0;
                }
            ...
            Object.keys(mafiaDecisions).forEach(mId => {
                let counts = mafiaDecisions[mId].votesCount;
                let max = 0;
                Object.keys(counts).forEach(act => {
                    if (counts[act] > max) {
                        max = counts[act];
                        mafiaDecisions[mId].action = act;
                    }
                });
            });
        }
        ...
        Object.keys(mafias).forEach(mId => {
            ...
            // Evento CRISIS: todos pierden $200
            if (game.currentEvent?.code === "CRISIS") {
                mafias[mId].money = Math.max(0, mafias[mId].money - 200);
            }
        });

        // Evento LAVADO: +$10 por punto de influencia (se aplica después del loop)
        if (game.currentEvent?.code === "LAVADO") {
            Object.keys(mafias).forEach(mId => {
                let bonus = mafias[mId].influence * 10;
...
function renderEndScreen() {
    ...
    if (mafiasArr.length > 0) {
        document.getElementById('winner-mafia-name').innerText = mafiasArr[0].name;
    }

    let podiumUi = document.getElementById('final-podium');
    podiumUi.innerHTML = "";
    ...
    if (stats) {
        ...
        Object.keys(stats.traiciones || {}).forEach(mId => {
            if (stats.traiciones[mId] > maxTraiciones) {
                maxTraiciones = stats.traiciones[mId];
                traidorId = globalGameState.mafias[mId]?.name;
            }
        });
        Object.keys(stats.cooperaciones || {}).forEach(mId => {
            if (stats.cooperaciones[mId] > maxCooperas) {
                maxCooperas = stats.cooperaciones[mId];
                confiableId = globalGameState.mafias[mId]?.name;
            }
        });
